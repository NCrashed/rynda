...
impl<T> ShaderBuffer<T> {
    ...
    pub fn new(size: usize) -> Self {
        ...
        unsafe {
            ...
            gl::BufferData(gl::SHADER_STORAGE_BUFFER, buffer_size as isize, dummy_data.as_ptr() as *const c_void, gl::DYNAMIC_COPY);
...
impl ShaderBuffer<PointerColumn> {
    ...
    pub fn from_pointermap(volume: &RleVolume) -> Self {
        ...
        unsafe {
            gl::GenBuffers(1,&mut id); 
            gl::BindBuffer(gl::SHADER_STORAGE_BUFFER, id);
            gl::BufferData(gl::SHADER_STORAGE_BUFFER, buffer_size as isize, volume.pointers as *const c_void, gl::DYNAMIC_COPY);
            gl::BindBuffer(gl::SHADER_STORAGE_BUFFER, 0);
        }
