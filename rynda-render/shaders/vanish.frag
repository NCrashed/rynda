...
void main() {
    ...
    float s4 = -0.5;
    if (segment_frag < 1.0) {
        uv.x = 0.5 * (ys - vp_point.y) / abs(xs - vp_point.x) + s1;
        uv.y = xs - vp_point.x;
        color = texture( segment_right, uv );
    } else if (segment_frag < 2.0) {
        uv.x = 0.5 * (xs - vp_point.x) / abs(ys - vp_point.y) + s2;
        uv.y = ys - vp_point.y;
        color = texture( segment_front, uv );
    } else if (segment_frag < 3.0) {
        uv.x = 0.5 * (ys - vp_point.y) / abs(xs - vp_point.x) + s3;
        uv.y = xs - vp_point.x;
        color = texture( segment_left, uv );
    } else {
        uv.x = 0.5 * (xs - vp_point.x) / abs(ys - vp_point.y) + s4;
        uv.y = ys - vp_point.y;
